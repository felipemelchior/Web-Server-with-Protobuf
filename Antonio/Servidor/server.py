...
def connected(client, addr):
	...
	Trata a mensagem recebido com base nos valores dele (ex: "GET /")
		...
		if message:
			...
			if signature == message.getSignature():
				if message.getCommand() == "GET":
					response = getMethod(message.getUrl(), message.getClientId(), message.getClientInfo(), key)
					communication.sendMessage(client, response)

				elif message.getCommand()  == "POST":
					response = postMethod(message.getUrl(), message.getClientId(), message.getClientInfo(), message.getContent(), key)
					communication.sendMessage(client, response)
